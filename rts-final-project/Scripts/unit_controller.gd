...
func _try_command_unit ():
	...
	if target != null:
		if selected_unit.is_in_group("Healer"):
			if target.team == Unit.Team.PLAYER and target.cur_hp < target.max_hp:
				selected_unit.set_attack_target(selected_unit, target)
